...
commit: commit.tree.sha}
  ...
  const informationToReport = uniquePullRequests.filter((pr) =>
    pr.labels.find((label) => label.name === core.getInput('label', {required: true}))).map((pr) => {
      ...
      const loomLinks = pr.body.match(/\[.*\]\((https:\/\/www.loom.com.*)\)/).slice(1, -1);
